...
def listener(arg):
    ...
    rospy.init_node('ochang_global_path_publisher', anonymous=True)
    ...
    while not rospy.is_shutdown():
        ...
        if loaded == 0:
            ...
            while(line):
               ...
               temp_pose.pose.position.x = x-361001.412425
               temp_pose.pose.position.y = y-4065821.07176
...
if __name__ == '__main__':
    try:
        if len(sys.argv) < 2:
                print("usage: global_path_publisher.py {ochang_testpath1 or f}")
        else:
            listener(sys.argv[1])
